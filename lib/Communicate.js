...
class Communicate  {
	...
	async sendto(packet, ipaddress, port, timeout ) {
		...
		if( this.dgramSocket === undefined ) {
			...
			this.dgramSocket.on('error', (err) => {
				console.log('**> dgramSocket error'+err)
				//Util.debugLog(this.homey.settings.get('DebugSettings'),'**> dgramSocket error'+err);
				clearTimeout(this.tm);
				this.tm=undefined;
		...
		return new Promise( function(resolve, reject ) {
			...
			this.tm = setTimeout( function() {
				clearTimeout(this.tm);
				this.tm=undefined;
				console.log('**> dgramSocket timeout')
				//Util.debugLog(this.homey.settings.get('DebugSettings'),'**> dgramSocket timeout');
				if( this.dgramSocket !== undefined ) {
					this.dgramSocket.close();
					this.dgramSocket = undefined;
				}
				reject( Error('errors.sending timeout'));
	...
	async send_packet(command, version, payload) {
		...
		console.log('->send_packet: payload='+Util.asHex(payload))
		...
		// send packet
		console.log('send package to: '+this.ipAddress)
	...
	*/
	...
	async discover( timeout, local_ip_address, device_ip_address ) {
		...
		if( local_ip_address == null ) {
			throw new Error('errors.discover_local_ip')
		}
			...
			let tm = setInterval( function() {
				...
				this.send_packet(0x6a, false, payload)
					.then (
						...
						rejection => {
							console.log('**> _check_data - reject: '+rejection)
							//Util.debugLog(this.homey.settings.get('DebugSettings'),'**> _check_data - reject: '+rejection)
							clearInterval(tm);
							tm=null;
							reject('aborted');
						})
					.catch( err => {
						console.log('**> _check_data - catch: '+err)
						//Util.debugLog(this.homey.settings.get('DebugSettings'),'**> _check_data - catch: '+err)
						clearInterval(tm);
						tm=null;
						reject( err );
					})
	...
	async checkRFData() {
		...
		if ( resp[0] != 1 ) {
			throw new Error('errors.no_key_detected')
		}
	}
	...
	*/
	...
	async sp1_set_power_state(mode) {
		...
		try {
			...
			//throw new Error(Homey.error('errors.invalid_response'))
		}
	}
