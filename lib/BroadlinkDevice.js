		...
		this._communicate.auth()
			.then( (authenticationData ) => {
				...
				this.setSettings( newSettings )
					...
					.catch( err => {
						Util.debugLog(this.homey.settings.get('DebugSettings'),'**> settings error, settings not saved *');
					})
			})
			.catch( err => {
				Util.debugLog(this.homey.settings.get('DebugSettings'), '**> authentication error: ' + err);
			})
	}
